...
namespace Jurassic.WebFrame.Controllers
{
    ...
    public class FunctionsController : BaseController
    {
        ...
        /// <summary>
        /// 获取所有系统自动分析出的DLL模块中的控制器和方法名
        /// </summary>
        /// <returns></returns>
        public JsonResult GetAllControllerActions()
        {
            var functionMgr = new DataManager<AppFunction>();

            FunctionInitializer initializer = new FunctionInitializer(functionMgr);
            initializer.InitFunctions(MvcApplication.Assemblys);

            return Json(functionMgr.GetAll()
                .OrderBy(d => d.Ord)
                .ThenBy(d => d.Id), JsonRequestBehavior.AllowGet);
        }
    }
