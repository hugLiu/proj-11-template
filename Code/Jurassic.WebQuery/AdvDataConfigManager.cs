...
namespace Jurassic.WebQuery
{
    ...
    public class AdvDataConfigManager
    {
        ...
        /// <summary>
        /// 根据标签属性获取对应的配置文件中对应的配置项
        /// </summary>
        /// <param name="attr">标签属性</param>
        /// <returns>属性配置项</returns>
        public static AdvDataConfigItem GetPropertyConfig(CatalogExtAttribute attr)
        {
            string className = attr.Property.DeclaringType.Name;
            string propName = attr.Property.Name;
        ...
        public static AdvDataConfigItem GetDataConfigItem(string className, string propName)
        {
            AdvDataConfigItem foundItem = null;
            var classConfig = _configCache.FirstOrDefault(cfg => cfg.ClassName.Equals(className));
            if (classConfig == null)
