...
namespace Jurassic.Com.OfficeLib
{
    ...
    public class ExcelHelper : IDisposable, IExcelHelper
    {
        ...
        public DataTable ExcelToDataTable(string sheetName, bool isFirstRowColumn)
        {
            ...
            if (firstRow == null)
                firstRow = sheet.GetRow(sheet.FirstRowNum);
            ...
            else
            {
                ...
                var cols = Enumerable.Range(firstRow.FirstCellNum, cellCount).Select(i => new DataColumn(_excelColNumArr[i])).ToArray();
        ...
        /// <summary>
        /// 将excel中的数据导入到DataTable中,指定逻辑上的第一行
        /// </summary>
        /// <param name="sheetName">标签名称</param>
        /// <param name="isFirstRowColumn">第一行是否列标题</param>
        /// <param name="firstRowNum">起始行号（从0开始)</param>
        /// <param name="firstColumnNum">起始列号（从0开始)</param>
        /// <returns>DataTable</returns>

        public DataTable ExcelToDataTable(string sheetName, bool isFirstRowColumn, int firstRowNum, int firstColumnNum)
        {
            ISheet sheet = null;
            int startRow = 0;
            ...
            {
                firstRow = sheet.GetRow(sheet.FirstRowNum);
                firstColumnNum = firstRow.FirstCellNum;
            }
            if (isFirstRowColumn)
            {
                for (int i = firstColumnNum; i < firstRow.LastCellNum; ++i)
                {
                    ICell cell = firstRow.GetCell(i);
                    if (cell != null)
                    {
            ...
            {
                //如果是不规则表格，则用A,B,C...作列名
                var cols = Enumerable.Range(firstColumnNum, firstRow.LastCellNum - firstColumnNum).Select(i => new DataColumn(_excelColNumArr[i])).ToArray();
                dt.Columns.AddRange(cols);
            ...
            for (int i = startRow; i <= rowCount; ++i)
            {
                ...
                for (int j = firstColumnNum; j < row.LastCellNum; ++j)
                {
                    if (row.GetCell(j) != null) //同理，没有数据的单元格都默认是null
                        dataRow[j - firstColumnNum] = GetCellValue(row.GetCell(j));
                }
                dt.Rows.Add(dataRow);
            }
            return dt;
