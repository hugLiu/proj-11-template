...
namespace Jurassic.AppCenter.Resources
{
    ...
    public static class ResHelper
    {
        ...
        {
            SortedDictionary<string,string> defProps = new SortedDictionary<string, string>();
            var coll = GetCollection(CurrentCultureName);
            foreach (string key in coll.AllKeys)
            {
                if (key.Contains("-")) continue;
                defProps.Add(key,  coll[key]);
            }

            return String.Format("var {0}={1};", varName, JsonHelper.ToJson(defProps));
        }
