    ...
    public static class JMvcExtensions
    {
        ...
        public static string FunctionAction(this UrlHelper url, AppFunction func)
        {
            ...
            foreach (var p in func.Arguments)
            {
                ...
                if (Regex.IsMatch(p.Value, "^[\\w|\\d]+$")) //valuepattern不是正则表达式
                {
                    dict[p.Key] = p.Value;
                }
            }
        ...
        /// <summary>
        /// 执行AppFunction的功能，返回执行结果
        /// </summary>
        /// <param name="html"></param>
        /// <param name="func"></param>
        /// <returns></returns>
        public static MvcHtmlString FunctionAction(this HtmlHelper html, AppFunction func)
        {
            if (func.ControllerName.IsEmpty())
            {
                return null;
            ...
            dict["area"] = func.AreaName;
            return html.Action(func.ActionName, controller, dict);
        }
...
{3}
        ...
        /// <param name="func">系统功能</param>
        /// <returns></returns>
        public static MvcHtmlString RenderFunction(this HtmlHelper html, AppFunction func)
        {
            if (!func.ControllerName.IsEmpty() && !func.ActionName.IsEmpty())
            {
                return html.FunctionAction(func);
            }
            else if (!func.Location.IsEmpty())
            {
                return html.Partial(func.Location, func);
            }
        ...
        internal static void ClearLoginModel(this BaseController controller)
        {
            ...
            controller.RemoveCookie("startPage" + controller.Request.Url.Port);
        }
